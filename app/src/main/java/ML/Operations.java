...
import ML.audio.*;
import ML.audio.feature.*;
import ML.classify.speech.HiddenMarkov;
import ML.classify.speech.vq.Codebook;
import ML.classify.speech.vq.Points;
import ML.db.*;
import ML.util.*;
...
public class Operations {
	...
	int samplePerFrame = 256;// 23.22ms
	...
	public void generateCodebook() throws Exception {
		System.out.println("generateCodebook");
		...
		for (int i = 0; i < wavFiles.length; i++) {
			for (int j = 0; j < wavFiles[i].length; j++) {
				...
				//todo extractFeatureFromFile
				//String listformat = AudioSystem.getAudioFileFormat(wavFiles[i][j]).toString();
				//System.out.println("check for list" + listformat);
	...
	public void hmmTrain() throws Exception {
		...
		// extract features
		wavFiles = trainTestWavs.readWaveFilesList();
		words = trainTestWavs.readWordWavFolder().toArray(new String[0]);
		for (int i = 0; i < wavFiles.length; i++) {
			...
			for (int j = 0; j < wavFiles[i].length; j++) {
				System.out.println("Currently :::" + wavFiles[i][j].getAbsoluteFile());
				FeatureVector feature = extractFeatureFromFile(wavFiles[i][j]);
				System.out.println("++++++Feacture extract successfully++++++" + currentWord);
